...
def dead_lock_samples_id(his_samples, my_expertise, my_storage, available, allsamples):
    dead_lock_samples = []
    if len(his_samples)==0:
        for sample_id, sample_data in allsamples.items():
            if sample_data['carried_by'] != 1:
                for molecule, cost in sample_data['cost'].items():
                    ...
                    if my_needs  > available[molecule]:
                        dead_lock_samples.append(sample_id)
        print_debug('dead_lock_samples:{}'.format(dead_lock_samples))
    return dead_lock_samples

def ready_to_submit_molecule(his_samples, his_storage, his_expertise, allsamples):
    ...
    for sample_id in his_samples:
        ...
        for molecule, cost in allsamples[sample_id]['cost'].items():
            print_debug('{}:cost:{},exp:{},store_left:{}'.format(molecule,cost,his_expertise[molecule],storage_left[molecule]))
...
def all_current_needs(my_samples, his_samples, my_submit_samples, my_expertise, his_expertise, my_storage, his_storage, available, allsamples):
...
def get_min_common_needs(my_expertise, my_storage, my_samples_id, available, allsamples):
...
def is_expertise_enough_for_my_sample(my_expertise, sample_id, allsamples):
    ...
    for molecule, cost in allsamples[sample_id]['cost'].items():
        ...
        if cost > (max_molecule + my_expertise[molecule]):
            print_debug("sample:{} {}:cost({}) > exp({}) + {}".format(sample_id, molecule, cost, my_expertise[molecule],max_molecule))
            return False
        if molecule_limit < 0:
            print_debug("sample:{} {}:cost({}) + exp({}) > {}".format(sample_id, molecule, cost, my_expertise[molecule],molecule_limit))
            return False
    return True
...
def validated_samples_id(expertises, sample_ids, his_samples, allsamples):
    validated_samples = []
    my_expertise = expertises[0]
    his_expertise = expertises[1]
...
def best_sample_for_science_project(projects_needs, sample_candidates, myexpertise, allsamples):
    best_samples = []
...
def cal_best_rank(available_storage, my_expertise, my_samples, undiagnosed_samples, available):
    not_completed_samples = len(my_samples) - len(undiagnosed_samples)
    rank = 1
    if sum(expertises[0].values()) > 10 and available_storage > 4 and not_completed_samples < 1:
        rank = 3
    elif sum(expertises[0].values()) > 5 and available_storage > 1 and not_completed_samples < 2:
        rank = 2
    return rank
...
def cal_best_molecule(sorted_my_samples, submit_samples, storage_left, available, my_expertise, his_current_needs, allsamples):
    ...
    for working_sample_id in collecting_samples:
        ...
        for molecule in sorted_available_molecule:
            ...
            print_debug("getting {}:needs:{},cost:{},storage_left:{},exp:{},available:{}"
                        .format(molecule.upper(),needs,cost,storage_left[molecule],my_expertise[molecule],available[molecule]))
    ...
    if break_samples:
        for working_sample_id in break_samples:
            ...
            for molecule in sorted_available_molecule:
                ...
                needs = cost - storage_left[molecule] - my_expertise[molecule]
                print_debug("getting {}:needs:{},cost:{},storage_left:{},exp:{},available:{}"
                            .format(molecule.upper(), cost, storage_left[molecule], my_expertise[molecule], needs, available[molecule]))
                if available[molecule] != 0 and needs > 0:
                    return molecule
    
    #start to collect his needs to prevent 
    if his_current_needs:
        sorted_his_current_needs_key = [k for k,v in sorted(his_current_needs.items(), key=lambda x: x[1], reverse=True)]
...
while True:
    ...
    for i in range(sample_count):
        ...
        sample_data['expertise_gain'] = expertise_gain
    ...
    valid_cloud_samples = validated_samples_id(expertises, cloud_samples, his_samples, samples)
    undiagnosed_samples = undiagnosed_samples_id(sorted_my_samples, samples)
    validated_samples = validated_samples_id(expertises, sorted_my_samples, his_samples, samples)
    submit_samples, storage_left = ready_for_lab_samples_id(expertises[0], storages[0], sorted_my_samples, samples)
    get_min_common_needs(expertises[0], storages[0], sorted_my_samples, available, samples)
    my_current_needs, his_current_needs = all_current_needs(sorted_my_samples, his_samples, submit_samples, expertises[0], expertises[1], storages[0], storages[1], available, samples)
    ...
    if targets[0] == 'SAMPLES':
        rank = cal_best_rank(available_storage, expertises[0], my_samples, undiagnosed_samples, available)
        if len(my_samples) == 3:
    ...
    if targets[0] == 'DIAGNOSIS':
        ...
        #send invlid samples back to cloud
        ...
        if len(valid_cloud_samples)!=0:
            ...
            best_samples = best_sample_for_science_project(projects_needs, sample_candidates, expertises[0], samples)
    ...
    if targets[0] == 'MOLECULES':
        ...
        molecule_tobe_collect = cal_best_molecule(sorted_my_samples, submit_samples, storage_left, available, expertises[0], his_current_needs, samples)
